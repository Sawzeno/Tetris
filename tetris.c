...
*/
  //-----------------------------------------------------------------Rendering  
    ...
    //memcpy(buffer.b + buffer.len, screen.buffer, screen.rows * screen.cols);
    //buffer.len += screen.rows * screen.cols;
...
void move(Object *object, int x, int y) {
  ...
  if (object->x + x >= 0 && object->x + x < screen.rows && object->y + y >= 0 && object->y + y < screen.cols)
  {
